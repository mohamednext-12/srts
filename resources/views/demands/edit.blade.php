                ...
                <div class="page-title-box">
                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            ...
                            <li class="breadcrumb-item active">{{__('Modifier une Demande')}}</li>
                        </ol>
                    </div>
                    <h4 class="page-title">{{__('Modifier une Demande')}}</h4>
                </div>
            </div>
        </div>
        <!-- end page title -->
            ...
            <div class="col-lg-12">
                <div class="card border">
                    <div class="card-body">
                        ...
                        <form id="form" class="needs-validation"  method="POST" action="{{ route('demands.update',$demand->id) }}">
                            @csrf
                            @method('put')
                            <div class="form-row">
                                <div class="form-group mb-3 col-md-6">
                                    <label for="name_arab">{{ __('Quantité') }}</label>
                                    <input name="qty" type="text" class="form-control" id="qty" @error('qty') is-invalid @enderror placeholder="{{ __('Quantité') }}" value="{{$demand->qty}}" required>
                                    @error('qty')
                                ...
                                <div class="form-group mb-3 col-md-6">
                                    ...
                                    <input name="date" type="date" class="form-control" id="date" @error('date') is-invalid @enderror placeholder="{{ __('Date') }}" value="{{$demand->date}}" required>
                                    @error('date')
                                    <span class="invalid-feedback" style="display: block !important;" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
